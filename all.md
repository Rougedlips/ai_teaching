...
V2.O功能
现在需要增加新的一套体系功能，涉及学生端、教师端和管理端。这一套增加后需要再github上增加分支V0.2-综合项目组队申请。该体系功能主要为：1. 管理端增加一个管理页面（综合项目管理），1个页签可以管理今年发布的课题库（课题名称、指导老师、课题资料、课题附件，开设方向和是否发布），第2个页签可以看到数据库所有学期的组队情况，并且可以搜索和筛选，列表属性包括，队伍编号、课程方向、课题名称、指导老师、队伍有哪些学生，操作包括编辑，编辑队伍信息包括增删学生、增删队伍）第3个页签是综设申请设置，包括综设组队是否开启，在开启时间段内学生才可以组队选题，队伍人数上限和下限设置。第4个页签是班级课题设置，需要管理员手动为每个系统里已经存在的教学班配置课题库里的课题，2. 老师端增加综合项目选题管理一级导航，里面页签1：教师题目管理，和管理端的课题库一样，教师可以新增题目，新增题目可以进入课题库，可以选择是否发布，发布后学生才能选择，页签2：学生组队申请列表，可以看到本学期申请老师的学生团队列表，包括申请信息，申请附件，操作有接受和拒绝。3. 学生端将当前我的队伍改成综合项目申请管理一级导航，里面有页签1，即当前的我的队伍的页面，可以新增队伍（新增人默认成为队长），添加队员（手动输入学号和姓名搜索），队伍限制为：1名系统里的同学只能在1个队伍里，同一个专业方向的学生才可以在一个队伍里。队伍可以添加的人数根据管理员设置的限制。页签2是申请课题页面，该队伍有自己的方向，所以只能申请该方向发布的课题，申请后等待老师同意，老师同意后该组队伍进入老师课题关联的班级学生列表和本班级组队情况表